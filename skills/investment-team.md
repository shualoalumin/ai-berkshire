...
在启动团队前，检索该公司是否有历史决策记录：
- `reports/{公司名}/{公司名}-thesis.md`（投资论文与追踪记录）
- `实盘记录/实盘操作记录.md` 中该公司的历史交易
- `memory/decisions/` 中该公司过往的团队分析记录（参见 `memory/decisions/README.md`）

若存在历史记录，在团队简报中注明：
- 上次分析的日期、结论、核心分歧点
- 上次的关键假设中，哪些已被验证/证伪（如有 thesis 追踪记录）
- **本次分析必须明确回应**："相比上次，什么变了？" 而非重新从零分析
...
**在关闭团队前**，team-lead 从4份报告中提炼 2-3 个**关键分歧点**（如"估值是否有安全边际"、"护城河是否稳固"、"管理层是否值得信任"）。这是四大师框架的核心价值——不是四段分析拼接，而是让立场真实冲突。

对每个分歧点：
1. **摘录多方论据**：哪个分析师（段永平/巴菲特视角通常偏乐观）持什么立场，引用其报告原文数据
2. **摘录空方论据**：哪个分析师（芒格/李录视角通常偏谨慎）持什么立场，引用其报告原文数据
3. **team-lead 追问**：向乐观一方追问"最强反例是什么"，向谨慎一方追问"是否忽视了积极因素"
4. **team-lead 裁决**：给出倾向性判断，但**不强行消除分歧**——如果分歧本身无法用现有数据消解，明确标注"存在真实分歧，需投资者自行判断"

若某个分歧点涉及可通过追加搜索验证的事实（而非价值判断），可选择性地向对应 Agent 追加一轮 SendMessage 提问，等待回复后再裁决（不强制，视时间成本决定）。

**输出交锋记录表**：

| 分歧点 | 多方论据（来源） | 空方论据（来源） | team-lead 裁决 |
|--------|-----------------|-----------------|----------------|
| | | | |
...
#### 5. 结构化交锋 + 投资论点（Bull vs Bear）
- 交锋记录表（第五步半产出，逐条列出分歧点/多方论据/空方论据/裁决）
- 🟢 看多逻辑（5-7条，含交锋后仍站得住的论据）
- 🔴 看空逻辑（5-7条，含交锋后仍站得住的论据）
- 若存在无法消解的真实分歧，单独标注"⚠️ 未消解分歧"，不強行给假共识
...
按 `memory/decisions/README.md` 规范，将本次分析的关键结论追加到 `memory/decisions/{公司名}.md`：
- 分析日期、综合评分、核心结论
- 结构化交锋中的关键分歧点与裁决
- 若用户后续执行买入，`thesis-tracker` 会在此记录基础上建立可追踪论文

这一步让下次分析同一家公司时，团队能"记得"上次的判断和分歧，而不是每次从零开始。
