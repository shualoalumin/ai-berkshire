...
检查 `memory/decisions/{公司名}.md` 是否存在（参见 `memory/decisions/README.md`）。
若存在，读取最新记录：
- 若来自 `/investment-team` 的历史分析——将其评分、结构化交锋分歧点作为建立论文的参考起点
- 若已有历史 thesis 追踪记录——重点核对上次"后续验证"字段是否已回填
...
按 `memory/decisions/README.md` 规范，在 `memory/decisions/{公司名}.md` 追加一条记录：
建立日期、核心论文摘要、核心假设清单。供未来 `/investment-team` 等分析类 skill 检索。
...
同步追加到 `memory/decisions/{公司名}.md`：本次健康度评分、核心变化、"上次判断的后续验证"
（即回填 B3 表格中每条假设相比上次的变化，供下次分析或建仓其他公司时参考同类判断模式）。
